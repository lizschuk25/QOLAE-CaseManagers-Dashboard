...
// Purpose: SSOT-compliant session validation for CaseManagersDashboard
...
const COOKIE_NAME = 'qolaeCaseManagerToken';
const LOGIN_REDIRECT = '/caseManagersLogin';
...
async function sessionMiddleware(request, reply) {
  ...
  if (urlPath.startsWith('/caseManagersCompliance')) {
    return;
  }
  ...
  try {
    ...
    const rawResponse = await ssotFetch('/auth/validateAndRefreshSession/caseManagers', {
      method: 'POST',
